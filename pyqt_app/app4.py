...
import sys
from greeting_ui import Ui_MainWindow
...
class GreetMainWindow(QMainWindow):
    ...
    def _initEventHandlers(self):
        self.ui.greetButton.clicked.connect(self.handle_greet)

    def handle_greet(self):
        name = self.ui.nameTextBox.text()
        if name.strip() == "":
            name="John Doe"
        self.ui.messageLabel.setText(f"Hello, {name}")
        self.ui.messageLabel.adjustSize()
